...
export class VoiceController {
  ...
  async joinRoom(@Request() req, @Param('roomId') roomId: string) {
    try {
      ...
      if (!result?.token || !result?.liveKitRoomId) {
        this.logger.error(
          `❌ Invalid join result for room ${roomId}: missing token or LiveKit room ID`,
        );

        throw new HttpException(
          {
            success: false,
            message: 'Failed to prepare LiveKit voice connection',
            error: 'LIVEKIT_JOIN_DATA_MISSING',
          },
      ...
      if (!this.liveKitService.isAvailable()) {
        this.logger.error(
          `❌ LiveKit unavailable while joining room ${roomId}`,
        );
      ...
      // The token is already generated by VoiceService.
      // Do NOT generate another token here.
      ...
      this.logger.error(
        `❌ Join room error: ${error?.message || error}`,
        error?.stack,
      );
